...
const NORMAL_TTL_SEC        = parseInt(Deno.env.get("NORMAL_TTL_SEC")          ?? "1800", 10);
const HOT_TTL_SEC           = parseInt(Deno.env.get("HOT_TTL_SEC")             ?? "30", 10);
const HOT_WINDOW_BEFORE_MIN = parseInt(Deno.env.get("HOT_WINDOW_BEFORE_MIN")   ?? "30", 10);
const HOT_WINDOW_AFTER_MIN  = parseInt(Deno.env.get("HOT_WINDOW_AFTER_MIN")    ?? "30", 10);
...
// A stream is genuinely on air when YouTube has recorded an actualStartTime and
// has NOT yet recorded an actualEndTime. This is the authoritative signal — far
// more reliable than search.list?eventType=live, which is eventually-consistent
// and routinely lags minutes (or fails entirely) behind a broadcast going live.
function isOnAir(item: StreamItem): boolean {
  return !!item.actualStartTime && !item.actualEndTime;
}
...
function mapSearchItem(item: Record<string, unknown>): StreamItem {
  ...
  return {
    ...
    liveBroadcastContent: (snippet.liveBroadcastContent as "live" | "upcoming" | "none") ?? "none",
...
async function ytSearch(channelId: string, eventType: "live" | "upcoming"): Promise<StreamItem[]> {
...
async function fetchVideoDetailsMap(
  videoIds: string[],
): Promise<Map<string, { scheduledStartTime: string | null; actualStartTime: string | null; actualEndTime: string | null }>> {
  const map = new Map<string, { scheduledStartTime: string | null; actualStartTime: string | null; actualEndTime: string | null }>();
  ...
  const json = await res.json() as {
    items?: Array<{ id: string; liveStreamingDetails?: { scheduledStartTime?: string; actualStartTime?: string; actualEndTime?: string } }>;
  };
...
async function fetchFromSearch(channelId: string): Promise<FetchSearchResult> {
  ...
  // ~2 units: enrich BOTH buckets with liveStreamingDetails. We must enrich the
  // upcoming bucket too — that is the whole point of this fix: an item returned by
  // eventType=upcoming may already be on air (actualStartTime set) while
  // eventType=live still hasn't caught up.
  const allIds = [...new Set(
    [...liveItems, ...upcomingItems].map((v) => v.videoId).filter(Boolean),
  )];
  const detailsMap = await fetchVideoDetailsMap(allIds);

  const enrich = (item: StreamItem): StreamItem => {
    const d = detailsMap.get(item.videoId);
    return {
      ...item,
      scheduledStartTime: d?.scheduledStartTime ?? item.scheduledStartTime ?? null,
      actualStartTime:    d?.actualStartTime    ?? item.actualStartTime    ?? null,
      actualEndTime:      d?.actualEndTime      ?? item.actualEndTime      ?? null,
    };
  };

  const enrichedLive     = liveItems.map(enrich);
  const enrichedUpcoming = upcomingItems.map(enrich);

  // Reclassify by the authoritative actualStartTime/actualEndTime signal rather
  // than trusting which search bucket the item came from.
  const live: StreamItem[]     = [];
  const upcoming: StreamItem[] = [];
  const seen = new Set<string>();

  for (const item of [...enrichedLive, ...enrichedUpcoming]) {
    if (!item.videoId || seen.has(item.videoId)) continue;
    seen.add(item.videoId);
    if (isOnAir(item)) {
      live.push({ ...item, liveBroadcastContent: "live" });
    } else if (!item.actualEndTime) {
      // Not yet started and not ended → genuinely upcoming.
      upcoming.push({ ...item, liveBroadcastContent: "upcoming" });
    }
    // Items with an actualEndTime have ended → drop from both lists.
  }

  return { live, upcoming, units: 202 };
}
...
async function fetchVideoById(videoId: string): Promise<StreamItem | null> {
  ...
  const json = await res.json() as {
    items?: Array<{
      id: string;
      snippet?: { title?: string; liveBroadcastContent?: string; thumbnails?: Record<string, { url?: string }> };
  ...
  // Trust actualStartTime/actualEndTime over snippet.liveBroadcastContent, which can
  // still read "upcoming" for a few minutes after a broadcast has actually started.
  let liveBroadcastContent = (item.snippet?.liveBroadcastContent ?? "none") as "live" | "upcoming" | "none";
  if (actualStartTime && !actualEndTime) liveBroadcastContent = "live";
  else if (actualEndTime) liveBroadcastContent = "none";
  return {
    videoId:             item.id,
...
Deno.serve(async (req: Request): Promise<Response> => {
  ...
  if (!SUPABASE_URL || !SUPABASE_SERVICE_KEY || !YOUTUBE_API_KEY) {
    return jsonResp({ error: "Server misconfiguration: missing environment variables." }, 500);
  }
  ...
  let hotScheduledAt: Date | null = row?.hot_scheduled_at ? new Date(row.hot_scheduled_at) : null;
  ...
  if (row?.cached_at && nowMs - new Date(row.cached_at).getTime() < ttlMs) {
    ...
    return jsonResp({ live: row.live, upcoming: row.upcoming, cachedAt: row.cached_at, source: "cache", mode });
  }
  ...
  try {
    if (mode === "hot" && hotVideoId) {
      ...
      if (!video || video.liveBroadcastContent === "none") {
        ...
        hotScheduledAt = video.scheduledStartTime ? new Date(video.scheduledStartTime) : hotScheduledAt;
      }
    } else {
      const r = await fetchFromSearch(channelId);
      newLive = r.live; newUpcoming = r.upcoming;
