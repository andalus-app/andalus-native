-- =============================================================================
-- Phase 2E — wheelchair_accessible on mosques (shown as the wheelchair icon
-- right of "Parkering" in the masjid card → tooltip "Rullstolstillgänglig
-- ingång"; the icon only appears when this is TRUE).
...
--
-- The column is nullable BOOLEAN: TRUE = wheelchair-accessible entrance,
-- FALSE = explicitly not, NULL = unknown/not specified.
-- =============================================================================

ALTER TABLE mosques
  ADD COLUMN IF NOT EXISTS wheelchair_accessible BOOLEAN;

-- ── nearby_mosques: include wheelchair_accessible ───────────────────────────
...
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  SELECT
    ...
    m.latitude, m.longitude, m.opening_hours, m.parking_available, m.wheelchair_accessible,
    m.access_info, m.phone, m.website, m.prayer_times_url, m.image_url,
    (6371000 * 2 * asin(sqrt(
...
-- ── submit_mosque: accept + store wheelchair_accessible (default NULL) ──────
-- Drop the current 16-arg signature (…, p_prayer_times_url TEXT) before
-- recreating with the new trailing BOOLEAN param, so there's no ambiguous
-- overload for supabase-js named-arg calls.
...
BEGIN
  ...
  INSERT INTO mosques (
    ...
    latitude, longitude, opening_hours, parking_available, wheelchair_accessible, access_info,
    ...
    p_latitude, p_longitude, p_opening_hours, p_parking_available, p_wheelchair_accessible, p_access_info,
...
GRANT EXECUTE ON FUNCTION submit_mosque(
  TEXT, TEXT, TEXT, TEXT, DOUBLE PRECISION, DOUBLE PRECISION,
  JSONB, BOOLEAN, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, BOOLEAN
